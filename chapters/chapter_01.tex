...
reaction rate within an energy window related to the temperature of the stellar
environment. The reaction rate per particle pair is given by
\begin{equation}
\label{eq:reaction-rate}
    \langle\sigma\nu\rangle_{01} = \left(\frac{8}{\pi m_{01}}\right)^{1/2}
        \frac{1}{(kT)^{3/2}}\int_0^{\infty} E\sigma(E)e^{-E/kT}\,\rm{d}E,
\end{equation}
where $m_{01} = m_0m_1/(m_0 + m_1)$ is the reduced pass of the particle
pair~\cite{Iliadis}.
- reaction rate when using $N_A$ is called...?

In cases where a single narrow resonance dominates the cross section,
Eq.~\ref{eq:reaction-rate} may be simplified to
\[
