...
\begin{table}
    \begin{center}
        ...
        S[table-format=3.2]S[table-format=3.2,retain-explicit-plus]S[table-format=3.2]}
            ...
            \midrule
264           & 1.374 & 24.8 &  -3.5 &  +0.2 &  3.7 &  -4.1 &  +7.2 & 11.3 \\
...
The separator and experiment were set up to detect the $\alpha$
particles produced in the reaction. We can consider our separator to
then be tuned for $\mnuc{4}{He}^{2+}$. During our reaction, some of the
produced alphas could be in the $1+$ charge state, based on the
interactions with the target material. This additional distribution
could have an effect on the final acceptance measurements, since we are
assuming that all of the produced $\alpha$ particles enter into St.\
George in the $2+$ charge state.

The empirical charge state distribution for the interaction between the
target material and the produced $\alpha$ particles can modeled using
...
$\sim 1$~MeV/u, whereas the alphas produced are at energies of
...
for our \nuc{27}{Al} target gives a better than 97\,\% proportion of
